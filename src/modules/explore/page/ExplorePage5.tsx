...
export default function ExplorePage() {
  ...
  const onImageClick = useCallback((id: SetStateAction<null>) => {
    setImageID(id);
...
const stateCodeToImage: StateCodeToImage = {
  ...
  // useEffect(() => {
  //   if (!imageID) {
  //     // make the API call to get the image ID here
  //     fetch(" https://hackathon-wehack-23.herokuapp.com/api/v1/states/view?id=Kentucky ")
  //       .then((res) => res.json())
  //       .then((data) => {
  //         // set the retrieved image ID in the state
  //         setImageID(data.id);
  //       })
  //       .catch((error) => {
  //         console.error(error);
...
return (
    <div>
      ...
      {data.map(({ id, state_code }) => (
        <img
          key={state_code}
          className={stateCodeToImage[state_code]?.className || ''} 
          src={stateCodeToImage[state_code]?.src || ''}
          alt=""
        />
      ))}
            console.log(img);
